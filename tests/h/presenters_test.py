...
class TestDocumentHTMLPresenter(object):
    ...
    def test_web_uri_returns_via_front_page(self):
        """It doesn't strip https://via.hypothes.is if that's the entire URL."""
        for via_url in ('https://via.hypothes.is', 'https://via.hypothes.is/'):
    ...
    def test_web_uri_strips_via(self):
        """
        ...
        """
        for path in ('foo', 'http://example.com'):
